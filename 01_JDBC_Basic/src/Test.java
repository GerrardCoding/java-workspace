...
public class Test {
	 ...
	 public static void main(String[] args) {
		  ...
		  *  			   INSERT, UPDATE, DELETE문은 실행 후 반환값이 int값이 됨.
		  *  
		  *  * JDBC 연결 과정(순서 중요)
		  *  
		  ...
		  *  6-1) Resultset에 담겨있는 데이터들을 하나씩 추출해서 변수나, 자바객체에 옮겨 담는다.
		  *  6-2) 트랜잭션 처리(성공적으로 수행됐으면 COMMIN, 실패했으면 ROLLBACK)
		  *  
		 ...
		 // DB에 전달할 SQL문 작성. 마지막 세미콜론(;)은 포함하면 안됨.
		 String sql = "SELECT TNO, TNAME, TDATE FROM TEST";
		 
		 try {
			 
			 //1) ojdbc6.jar 파일이 등록안됐거나, 오타가 있을 경우
			 Class.forName("oracle.jdbc.driver.OracleDriver");
			 System.out.println("ORACLE JDBC드라이버가 로드됐습니다.");	
			 
			 //2) Connect 객체 생성(url(DB위치), username, password)
			 ...
			 //	  ResultSet의 next()메서드는 조회된 다음 레코드가 있는지 확인해서 있으면 true, false반환
